...
const BookDetails = () => {
    ...
    const fetchBookDetails = async () => {
      try {
        ...
        const formattedBook = {
          ...
          cover: volumeInfo.imageLinks?.thumbnail || 'https://via.placeholder.com/300x450',
        ...
        if (volumeInfo.categories?.[0]) {
          const recResponse = await fetch(
            `https://www.googleapis.com/books/v1/volumes?q=subject:${encodeURIComponent(volumeInfo.categories[0])}&maxResults=4&key=${GOOGLE_BOOKS_API_KEY}`
          );
          ...
          if (recResponse.ok) {
            ...
            const formattedRecs = recData.items
              ...
              .map(item => ({
                ...
                cover: item.volumeInfo.imageLinks?.thumbnail || 'https://via.placeholder.com/150x225'
              }));
  ...
  const handleAddToWishlist = () => {
    if (book) {
      ...
      if (!isInWishlist) {
        ...
        setShowMessage('Already in wishlist!');
      }
      setTimeout(() => setShowMessage(''), 3000);
    }
  };
  ...
  if (!book) {
    return (
      <div className="min-h-screen bg-[#4a919e] flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-[#212e53] mb-4">Book not found</h2>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center gap-1">
                      {Array(5).fill().map((_, i) => (
                        <span key={i} className={`text-xl ${i < book.averageRating ? 'text-yellow-400' : 'text-gray-300'}`}>
                          ★
                        </span>
                      ))}
                      <span className="text-sm text-gray-600 ml-2">({book.ratingsCount})</span>
                    </div>
                  </div>
